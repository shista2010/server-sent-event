...
func main() {
	...
	authorized.Use(HeadersMiddleware())
	authorized.Use(stream.serveHTTP())
	...
	authorized.GET("/stream", func(c *gin.Context) {
		...
		c.Stream(func(w io.Writer) bool {
			...
			ch := (c.Keys["clientChan"]).(ClientChan)
			if msg, ok := <-ch; ok {
				log.Println("client:", c.Keys["clientChan"])
				c.SSEvent("message", msg)
				return true
			}
			log.Println("End of listening")
			return false
		})
	})
